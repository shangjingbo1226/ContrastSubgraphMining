...
//padding a sentence to length l
string padding_sentence(string a, size_t l){
    int pad = l - a.length();
    for ( int i = 0 ; i < pad ; i ++ ) {
        a = a + " " ;
    }
...
inline FILE* tryOpen(const string &filename, const string &param)
{
    ...
    if (ret == NULL) {
        ...
        cerr << "[Warning] trying to open ../" << filename << " instead" << endl;
        string temp = "../" + filename;
        ret = fopen(temp.c_str(), param.c_str());
        if ( ret == NULL ) {
            cerr << "[Warning] alternative address failed" << endl;
        }
    }
    return ret;
}
...
{
    while (s.size() > 0 && iswspace(s.back())) {
        s.pop_back();
    }
    int ptr = 0;
    while (ptr < s.size() && iswspace(s[ptr])) {
        ++ ptr;
    }
    s = s.substr(ptr, s.size() - ptr);
    return s;
}
